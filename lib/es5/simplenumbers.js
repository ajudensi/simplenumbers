...
var SimpleNumbers = function () {
	...
	var SimpleNumbers = function SimpleNumbers(config) {
		...
		this.numberGenerator = function (max, start, stride) {
			...
			max = parseInt(max, 10) || parseInt(config.maximumNumber, 10);
			stride = parseInt(stride, 10) || parseInt(config.stride, 10) || 1;
		...
		this.fibo = function (typeConfig) {
			...
			for (var i = typeConfig.start; i < producedNumbers[producedNumbers.length - 1]; i++) {
